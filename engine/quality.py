...
def evaluate(article: dict) -> QualityReport:
    ...
    if found:
        ...
        score -= 35
    if article.get("lottery") and article.get("play") and not verified_rules(article["lottery"], article["play"]):
        errors.append("no verified rule for declared lottery/play")
        score -= 35
