...
BATCH_INDEX = ROOT / "articles" / "batches" / "CF300-20260813.json"
PATTERN = "LCM-CREATOR-cf300-20260813-*.json"
CASE_LABEL = "演示参数，不是真实开奖记录"
EXPECTED_PLAY_DISTRIBUTION = {
    "后二直选": 50,
    "后三直选": 55,
    "五星直选": 55,
    "定位胆": 40,
...
def test_creator_first_batch_300_all_pass_existing_approval_and_formal_inventory():
    packages = _packages()
...
def test_creator_first_batch_300_has_unique_seo_identity_and_no_intra_batch_duplicates():
    packages = _packages()
    assert len(packages) == 300
    assert len({row["article_id"] for row in packages}) == 300
    assert len({row["slug"] for row in packages}) == 300
    assert len({row["primary_keyword"] for row in packages}) == 300
    assert len({row["content_hash"] for row in packages}) == 300
...
def test_creator_first_batch_300_manifest_is_complete_and_inventory_only():
    packages = _packages()
    batch = json.loads(BATCH_INDEX.read_text(encoding="utf-8"))

    assert batch["batch_id"] == "CF300-20260813"
    assert batch["count"] == 300
    assert batch["style_family_count"] == 20
    assert batch["publication_status"].startswith("inventory_only")
    assert batch["website_ingestion_glob"] == f"articles/approved/{PATTERN}"
    assert len(batch["articles"]) == 300
    assert {row["article_id"] for row in batch["articles"]} == {
        row["article_id"] for row in packages
    }
    assert Counter(row["subject_play"] for row in packages) == Counter(EXPECTED_PLAY_DISTRIBUTION)
