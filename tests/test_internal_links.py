...
def test_dingweidan_cold_hot_prefers_omission_and_format_guide():
    plan = internal_links.plan_internal_links("LCM-IDEA-48eb8743fbbbad11", limit=3)
    ids = [target["target_article_id"] for target in plan["targets"]]
    assert ids[:2] == ["LCM-IDEA-07838cdf108296a7", "LCM-SMOKE-20260811-02"]
    anchors = {target["target_article_id"]: target["anchor_hint"] for target in plan["targets"]}
    assert anchors["LCM-IDEA-07838cdf108296a7"] == "分分彩定位胆遗漏技巧"


def test_last3_sum_connects_to_last3_span_before_broader_three_digit_content():
    plan = internal_links.plan_internal_links("LCM-IDEA-9ea1859cc88b2682", limit=3)
...
def test_no_forced_link_when_only_same_lottery_is_available():
    plan = internal_links.plan_internal_links("LCM-IDEA-bf5a9864b004ae17", limit=3)
